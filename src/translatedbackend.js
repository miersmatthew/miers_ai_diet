...
function getBmr(gender, weight, height, age){
    if (gender == "male"){
        var bmr = 88.362 + (13.397 * weight) + (4.799 * height * 100) - (5.677 * age);
    } else if (gender == "female"){
        var bmr = 447.593 + (9.247 * weight) + (3.098 * height * 100) - (4.330 * age);
    } else {
...
function getCalNeeds(activityLev, bmr){
    // Activity level assignments
    // 1.2 - "sedentary"
    // 1.375 - "lightly active"
    // 1.55 - "moderately active"
    // 1.725 - "very active"
    var calNeeds = bmr * activityLev
    return calNeeds
}

function getCalGoal(calNeeds, goal){
    if (goal == 'Gain Weight'){
        return calNeeds * 1.1
...
function backendHandle(reqJson){

    var food = [["Donuts [1]","250","T","F","F","15g","15g","31g","4g","A donut is a type of fried dough pastry typically shaped like a torus and sweetened with sugar or other sweeteners. It often contains a filling or topping, such as jam, chocolate, or frosting.","https://img.sndimg.com/food/image/upload/f_auto,c_thumb,q_55,w_860,ar_3:2/v1/img/recipes/38/38/68/wmk3BwJSK2g1NYdTaczo_DSC_0163.jpg"],
    ...
    var bList = food.filter(f => f[2] == "T");
    var lList = food.filter(f => f[3] == "T");
    var dList = food.filter(f => f[4] == "T");
    
    //lazy shuffle
    bList.sort( () => .5 - Math.random() );
